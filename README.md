...
一文字駒のスクリーンショットを、ブラウザ画面で確認しながらKIF/SFENに変換する手順です。Windowsでは `start_kif_ui.cmd` をダブルクリックして使います。
...
- 対応したのは「将棋クエスト」と「ぴよ将棋」の1枚駒、つまり一文字駒のスクリーンショットです。
...
初回だけPython環境が必要です。将棋クエスト一文字駒向けのモデルと学習用画像、ぴよ将棋一文字駒向けのモデルを同梱しています。別アプリや別スタイル向けに作り直す場合は、下の「モデルを用意する」のコマンドで別モデルを作成できます。
...
ぴよ将棋一文字駒のローカル画像とラベルで専用モデルを作り直す場合は、次のように実行します。
...
公開リポジトリには、CLI本体、ドキュメント、テスト、将棋クエスト一文字駒の学習用画像、対応する学習済みモデル、ぴよ将棋一文字駒の学習済みモデルを含めます。
...
The Shogi Quest and Piyo Shogi one-character models are already included. To rebuild the Shogi Quest model from the bundled images and labels, run:

```powershell
shogi-gazo train-model --screenshots-dir data\samples\screenshots_by_app_piece_style\将棋クエスト\一文字駒 --labels data\samples\labels\boards_by_app_piece_style\将棋クエスト\一文字駒 --out models\shogi_quest_ichimonji_piece_model.pkl --include-hands
```

To rebuild the Piyo Shogi model from local labeled screenshots, run:
...
The public repository includes the CLI, documentation, tests, Shogi Quest one-character training screenshots, labels, the corresponding trained model, and the Piyo Shogi one-character trained model.

These folders are generally local development outputs and are not intended for public distribution, except for the Shogi Quest one-character screenshot folder and bundled one-character models:

...
3. 初回だけ、学習済みモデルを `outputs\models\piece_model.pkl` に置きます。
4. 以後は `start_kif_ui.cmd` をダブルクリックします。
5. ブラウザが開いたら、画像ファイルを選ぶか、クリップボード画像を `Ctrl+V` で貼り付けます。
6. 盤面と持ち駒を目視確認してから、KIFまたはSFENをコピーします。

GitHub公開版には、大きな学習済みモデルや第三者アプリのスクリーンショットは同梱していません。別ユーザーへ配布する場合は、このリポジトリに加えて、作成済みの `piece_model.pkl` を `outputs\models\piece_model.pkl` として渡してください。モデルがない状態では、UIを開けても画像認識はできません。
...
- `piece_model.pkl` が見つからない場合は、`outputs\models\piece_model.pkl` にモデルを置いてください。

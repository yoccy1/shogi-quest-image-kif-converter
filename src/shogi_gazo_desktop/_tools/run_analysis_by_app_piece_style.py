...
from learned_piece_recognizer import build_model, hand_classifier_assets, load_model, recognize_image, save_model
...
def group_key(image_path: Path, screenshots_dir: Path) -> tuple[str, str]:
    ...
    piece_style = parts[1] if len(parts) >= 2 else "未分類"
    return app, piece_style
...
def prepare_review_labels(
    ...
    initial_label_rows = initial_rows()
    for image_path in image_paths:
        app, piece_style = group_key(image_path, screenshots_dir)
        destination = review_labels / app / piece_style / f"{image_path.stem}.json"
...
def write_group_summary(path: Path, out_dir: Path, rows: list[dict[str, Any]], groups: set[tuple[str, str]]) -> None:
    summary = []
    for app, piece_style in sorted(groups):
        group_rows = [row for row in rows if row["app"] == app and row["piece_style"] == piece_style]
        group_dir = out_dir / app / piece_style
        html = group_dir / "visual_review.html"
        summary.append(
            {
                "app": app,
                "piece_style": piece_style,
                "manifest_images": len(group_rows),
                "reports": len(list(group_dir.glob("*/piece_report.json"))),
                "labeled": sum(row["label_status"] == "教師ラベルあり" for row in group_rows),
                "initial": sum(row["label_status"] == "初期配置ラベル" for row in group_rows),
                "unlabeled": sum(row["label_status"] == "教師ラベル未作成" for row in group_rows),
                "visual_review_html": str(html),
                "visual_review_exists": html.exists(),
            }
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run recognition for screenshots grouped by app and piece style.")
    parser.add_argument("--screenshots-dir", type=Path, default=Path("tools/samples/screenshots_by_app_piece_style"))
    parser.add_argument("--labels-dir", type=Path, default=Path("tools/samples/labels/boards_by_app_piece_style"))
    parser.add_argument("--calibration-dir", type=Path, default=Path("tools/samples/screenshots_by_app_piece_style"))
    parser.add_argument("--out", type=Path, default=Path("tools/out/analysis_by_app_piece_style"))
    ...
    if args.model is None:
        args.model = args.out / "models" / "all_labeled_piece_style_model.pkl"
    ...
    for index, image_path in enumerate(image_paths, start=1):
        app, piece_style = group_key(image_path, args.screenshots_dir)
        groups.add((app, piece_style))
        kind = "初期配置" if is_initial_image(image_path) else "通常"
        sample_out = args.out / app / piece_style / image_path.stem / "piece_report.json"
        ...
        print(f"[{index}/{len(image_paths)}] {app}/{piece_style}/{image_path.stem}: {seconds:.3f}s", flush=True)

    write_manifest(args.out / "manifest.csv", manifest_rows)
    for app, piece_style in sorted(groups):
        group_dir = args.out / app / piece_style
        try:
    ...
    write_group_summary(args.out / "group_manifest_summary.json", args.out, manifest_rows, groups)
